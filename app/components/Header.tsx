    ...
    <header className="sticky top-0 z-40 bg-light-bg border-b border-slate-200 shadow-sm">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-1 group">
          <span className="text-2xl font-lora font-bold text-slate-900 tracking-wide">
            elpaso
        ...
        <div className="hidden md:flex items-center gap-8">
          {menuItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="text-slate-700 hover:text-terracotta transition-colors text-sm font-lora font-medium"
            >
              {item.label}
            </Link>
          ))}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 space-y-3">
            {menuItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="block px-3 py-2 rounded-lg text-slate-700 hover:bg-slate-100 hover:text-terracotta transition-colors font-lora font-medium"
